...
const PLUGIN_ID = "strapi-xlsx-impexp-plugin";
...
const ExportButton = () => {
  ...
  const [allowedExportCollections, setAllowedExportCollections] = react.useState([]);
  const [loadingConfig, setLoadingConfig] = react.useState(true);
  const [isExporting, setIsExporting] = react.useState(false);
  react.useEffect(() => {
    const fetchConfig = async () => {
      try {
        const response = await get2("/strapi-xlsx-impexp-plugin/config");
        let config;
...
Proceed?`;
    ...
    setIsExporting(true);
    const parts = location2.pathname.split("::");
    ...
    try {
      ...
      const response = await get2(`/strapi-xlsx-impexp-plugin/export?${query}`);
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(designSystem.Flex, { direction: "row", gap: 2, children: /* @__PURE__ */ jsxRuntime.jsx(designSystem.Button, { onClick: handleExport, disabled: isExporting, children: isExporting ? "Exporting..." : "Export" }) });
};
const ImportButton = () => {
  const { get: get2 } = admin.useFetchClient();
  const [allowedImportCollections, setAllowedImportCollections] = react.useState([]);
  const [loadingConfig, setLoadingConfig] = react.useState(true);
  ...
  const formatMessage = (message) => {
    return message.split("\n").map((line, index2) => /* @__PURE__ */ jsxRuntime.jsx("div", { style: { marginBottom: index2 > 0 ? "8px" : "0" }, children: line }, index2));
  };
  react.useEffect(() => {
    const fetchConfig = async () => {
      try {
  ...
  const uploadFile = async (file, contentTypeUid2) => {
    setIsImporting(true);
    try {
      const formData = new FormData();
      formData.append("file", file);
      formData.append("contentTypeUid", contentTypeUid2);
      ...
      if (result.success) {
        const successMessage = [
          ...
          ...result.errors.length > 0 ? [
            ...
            ...result.errors.map(
              (error) => `   • Row ${error.row}, Column "${error.column}": ${error.message}`
            )
          ] : []
        ].join("\n");
        ...
        const errorMessage = [
          ...
          "❌ Errors encountered:",
          ...result.errors.map(
            (error, index2) => `${index2 + 1}. Row ${error.row}, Column "${error.column}": ${error.message}`
          ),
          "",
          `Total processed: ${result.totalProcessed}`
        ].join("\n");
      ...
      showModal(
        "Error",
        `Error uploading file for entity: ${contentTypeUid2}
...
If the problem persists, check your connection and file format.`,
  ...
  if (loadingConfig) {
    return /* @__PURE__ */ jsxRuntime.jsx(designSystem.Flex, { direction: "row", gap: 2, children: /* @__PURE__ */ jsxRuntime.jsx(designSystem.Button, { disabled: true, children: "Loading..." }) });
  }
  if (!allowedImportCollections.includes(contentTypeUid)) {
    return null;
  }
  return /* @__PURE__ */ jsxRuntime.jsxs(designSystem.Flex, { direction: "row", gap: 2, children: [
    ...
    isModalOpen && /* @__PURE__ */ jsxRuntime.jsx(designSystem.Modal.Root, { open: isModalOpen, onOpenChange: closeModal, children: /* @__PURE__ */ jsxRuntime.jsxs(designSystem.Modal.Content, { children: [
      ...
      /* @__PURE__ */ jsxRuntime.jsx(designSystem.Modal.Body, { children: /* @__PURE__ */ jsxRuntime.jsx(designSystem.Field.Root, { children: /* @__PURE__ */ jsxRuntime.jsx(designSystem.Field.Label, { children: /* @__PURE__ */ jsxRuntime.jsx("div", { style: {
        whiteSpace: "pre-wrap",
        lineHeight: "1.5",
        fontFamily: "monospace",
        fontSize: "14px"
      }, children: formatMessage(modalMessage) }) }) }) }),
      /* @__PURE__ */ jsxRuntime.jsx(designSystem.Modal.Footer, { children: /* @__PURE__ */ jsxRuntime.jsx(designSystem.Modal.Close, { children: /* @__PURE__ */ jsxRuntime.jsx(designSystem.Button, { variant: isSuccess ? "default" : "tertiary", children: isSuccess ? "Great!" : "Close" }) }) })
    ] }) })
  ] });
};
const index = {
  register(app) {
    app.addMenuLink({
      ...
      Component: async () => {
        const { App } = await Promise.resolve().then(() => require("./App-DRvFn-VQ.js"));
        return App;
      }
    });
    app.registerPlugin({
  ...
  async registerTrads({ locales }) {
    return Promise.all(
      locales.map(async (locale) => {
        try {
          const { default: data } = await __variableDynamicImportRuntimeHelper(/* @__PURE__ */ Object.assign({ "./translations/en.json": () => Promise.resolve().then(() => require("./en-Bypqc6Bn.js")), "./translations/ru.json": () => Promise.resolve().then(() => require("./ru-DeWXOHzs.js")) }), `./translations/${locale}.json`, 3);
